...
- 🐛 **既知の不具合（修正予定 / 詳細は `IMPROVEMENT_PLAN.md` を参照）**
  - スマホ表示で建物を配置できない（`updateDisplay()` がモバイルに存在しない需要表示要素を参照して例外→描画ループ停止）
  - PC表示でメニューパネルの ✕ ボタンが効かず閉じられない（CSS詳細度により `.hidden` が上書きされる）
  - メニュー内の「設定」ボタンが機能しない（削除せず動作するよう修正予定）
